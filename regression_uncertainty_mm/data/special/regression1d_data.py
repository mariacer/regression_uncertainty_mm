...
class ToyRegression(Dataset):
    ...
    def __init__(self, train_inter=[-10, 10], num_train=20,
        ...
        if std > 0:
            ...
            if perturb_test_val:
                test_eps = rand.normal(loc=0.0, scale=std, size=(num_test, 1))
                test_y += test_eps
        ...
        if num_val is not None:
            ...
            if std > 0 and perturb_test_val:
                val_eps = rand.normal(loc=0.0, scale=std, size=(num_val, 1))
                val_y += val_eps
        ...
        self._input_norm_translate = np.mean(self._data['in_data'])
        self._input_norm_scale = np.std(self._data['in_data'])

        self._output_norm_translate = np.mean(self._data['out_data'])
        self._output_norm_scale = np.std(self._data['out_data'])
    ...
    def get_output_scale_translation(self):
        return self._output_norm_scale, self._output_norm_translate
